...
print(conf_matrix)
# extract components of the confusion matrix
tn, fp, fn, tp = confusion_matrix(Y_train, prediction_LR).ravel()

# predict the probability for class 1 (not just class label)
probs_LR=logreg.predict_proba(X_train)


# calculate AUROC
Auroc_score=roc_auc_score(Y_train, probs_LR[:,1])
print("AUROC score: {:.2f}".format(Auroc_score))
...
plt.figure(1)
# ROC curve components
fpr, tpr, thresholdsROC = roc_curve(Y_train, probs_LR[:,1])
#plot
plt.plot(fpr,tpr)
plt.title("ROC curve")
plt.xlabel("1-SPEC")
...
plt.figure(2)
# Components of the Precision recall curve
precision, recall, thresholdsPR = precision_recall_curve(Y_train, probs_LR[:,1])
# plot
plt.plot(recall,precision)
plt.title("PR curve")
plt.xlabel("SENS (Recall)")
